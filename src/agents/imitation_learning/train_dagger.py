...
from src.agents.imitation_learning.replay_buffer import ReplayBuffer
from src.envs import env_wrappers

config_flags.DEFINE_config_file(
    "config", "src/agents/imitation_learning/configs/bc_heightmap.py",
...
def main(argv):
  ...
  student_policy = config.student_policy_class(
  ...
  rewards, cycles, _ = replay_buffer.collect_data(teacher_policy,
                                                  teacher_policy,
                                                  config.num_init_steps,
  ...
  for step in range(config.num_iters):
    ...
    loss = student_policy.train_on_data(replay_buffer,
                                        batch_size=config.batch_size,
                                        num_steps=config.num_steps)
    student_policy.save(os.path.join(logdir, f"model_{step}.pt"))
    # Collect more data
    rewards, cycles, _ = replay_buffer.collect_data(student_policy,
                                                    teacher_policy,
                                                    config.num_dagger_steps,
    ...
    writer.add_scalar("Policy Training/MSE", loss, step)
